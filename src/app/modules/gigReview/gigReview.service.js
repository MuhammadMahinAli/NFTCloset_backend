...
export const getAllGigReviewService = async (gig) => {
  const reviews = await GigReview.find({gig}).populate("gig").populate("buyer").populate("designer");
  return reviews;
};
//get all review by designer
export const getGigReviewByDesignerService = async (designer) => {
  const reviews = await GigReview.find({designer}).populate("gig").populate("buyer").populate("designer");
  return reviews;
};
//get single review
export const getSingleGigReviewService = async (gig, id) => {
  const review = await GigReview.findOne({gig, _id: id}).populate("gig").populate("buyer").populate("designer");
  return review;
};
//update gig review
export const updateGigReviewService = async (payload) => {
  const result = await GigReview.findOneAndUpdate({_id: payload?.id}, payload, {new: true});
  if (!result) {
